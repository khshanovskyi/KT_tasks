...
public class CampaignStatusUpdateStrategy extends AbstractEventStrategy {
  ...
  protected CampaignStatusUpdateStrategy(ScheduledEventRepository eventRepository, CampaignRepository campaignRepository) {
  ...
  public ScheduledEvent process(ScheduledEvent event) {
    try {
      ...
      Object newCampaignStatus;
      if (preprocessedEvent.getProperties().containsKey(Constants.CAMPAIGN_STATUS))
        newCampaignStatus = preprocessedEvent.getProperties().get(Constants.CAMPAIGN_STATUS);
      else throw new RuntimeException("Cannot find campaign by the name %s".formatted(campaignName));
      campaign.setStatus((CampaignStatus) newCampaignStatus);
  ...
  public Pair<EventType, EventResourceType> getType() {
    return Pair.of(Pair.of(EventType.UPDATE_STATUS, EventResourceType.CAMPAIGN));
